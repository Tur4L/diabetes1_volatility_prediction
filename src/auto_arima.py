...
# dbs = pd.read_csv('./data/db_windowed_patient.csv')
# beta2_windows = dbs.groupby('window_id')

# ''' Looping over beta2 scores '''
# for window_id, window in beta2_windows:
#     '''Creating the data'''
#     window['timestamp'] = pd.to_datetime(window['timestamp'])
#     window.set_index('timestamp', inplace=True)
#     patient_id = window['patient_id'][0]
#     beta2_score = window['BETA2_score'][0]
